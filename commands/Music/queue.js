    ...
    sameVoiceChannel: false,
     ...
     let m = ")";
      ...
      if(!player.queue.current){
        return message.channel.send({embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`<:cross1:1074564046134718474> | There Is Nothing Playing.`)]})
      }
     ...
     if (pages.length <= 1) {
      return await message.reply(pages[page] ? `\`\`\`nim\n    ⬐ current track\n${getspaces(player.queue.current.title)}   ${convertTime(player.queue.current.duration - player.position)} left\n    ⬑ current track\n\n${pages[page]}\n\n   This is the end of the queue!\`\`\`` : `\`\`\`nim\n    ⬐ current track\n${getspaces(player.queue.current.title)}   ${convertTime(player.queue.current.duration - player.position)} left\n    ⬑ current track\n\n   This is the end of the queue!\`\`\``)
       ...
       let previousbut = new MessageButton().setCustomId("queueprev").setEmoji("<:seekbackward:1066040472377753703>").setStyle("SUCCESS");

            let nextbut = new MessageButton().setCustomId("queuenext").setEmoji("<:seekforward:1066040499137429585>").setStyle("SUCCESS");

            let stopbut = new MessageButton().setCustomId("queuestop").setEmoji("<:home:1066039685425676288>").setStyle("DANGER");
