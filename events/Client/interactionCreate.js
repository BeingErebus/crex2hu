...
module.exports = async (client, interaction) => {
 ...
 let color = interaction.guild.me.displayHexColor !== '#ff0000' ? interaction.guild.me.displayHexColor : client.config.embedColor;

  const music = new MessageEmbed();
     music.setColor("#2f3136")
...
if(!interaction.guild.me.permissionsIn(interaction.channel).has('VIEW_CHANNEL'))
   ...
   const But3 = new MessageButton().setCustomId("pause").setEmoji(!player.paused ? "<:play:1066040178138943530>" : "<:pause:1066040132253261905>").setStyle(!player.paused ? "SUCCESS" : "SECONDARY");
...
//server premium scopes interaction ^^
  ...
  let scot = 0;
  let slink = "https://discord.gg/ejSESjpgsF";
//Working & Checking Below 
  ...
  let spremown = await client.db.get(`spremown_${interaction.guild.id}`);
   ...
   await client.db.delete(`upremserver_${interaction.user.id}`)
    interaction.user.send({embeds: [new MessageEmbed().setColor(`#2F3136`).setDescription(`Your Premium Has Got Expired.\nTotal **\`${scot}\`** Servers [Premium](https://discord.gg/ejSESjpgsF) was removed.\nClick [here](https://discord.gg/ejSESjpgsF) To Buy [Premium](https://discord.gg/ejSESjpgsF).`)], components: [premrow]}).catch((err) => { });
  }
//User Prem Check Above ^^^
  ...
  { 
    ...
    try{
    await client.users.cache.get(`${us}`).send({embeds: [new MessageEmbed().setColor(`#2F3136`).setDescription(`Your Premium Has Got Expired.\nTotal **\`${scount}\`** Servers [Premium](https://discord.gg/ejSESjpgsF) was removed.\nClick [here](https://discord.gg/ejSESjpgsF) To Buy [Premium](https://discord.gg/ejSESjpgsF).`)], components: [premrow]}).catch((er) => { })
    }catch(errors) {
