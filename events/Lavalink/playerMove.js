...
module.exports = async (client, player, oldChannel, newChannel) => {
  ...
  if(newChannel)
  {
    ...
    player.pause(false)
    channel.send({embeds: [new MessageEmbed().setColor("#2f3136").setDescription(`Reconnected as **24/7** was **enabled**`)]}).then(message => {     setTimeout(function() {  message.delete()}, 2500)})
  }
  else
  {
