...
module.exports = class Util {
  ...
  async buildGreeting(member, type, config) {
    ...
    embed.setColor(config.embed.color ? config.embed.color : member.client.embedColor);
    ...
    if (!config.content && !config.embed.description && !config.embed.footer) {
      return { content: `<@${member.user.id}>`, embeds: [new MessageEmbed().setColor(this.client.embedColor).setDescription(`Hey ${member.displayName}, Welcome to the server <a:welcomer:1026898748460257281>.`)] };
    }
  ...
  async manageAfk(message, client){
    ...
    if (data) {
      if (message.author.id === data.Member) {
        await data.delete()
        return message.channel.send({embeds: [new MessageEmbed().setColor(message.client.color).setDescription(`I Removed Your AFK`)]})
      }
    }
    ...
    if (memberMentioned) {
      ...
      if (data) {
        message.channel.send({embeds: [new MessageEmbed().setColor(message.client.color).setDescription(`<@${memberMentioned.id}> went AFK <t:${Math.round(data.Time/1000)}:R>\n\nFor Reason: **${data.Reason}**`)]});
      } else {
         return
      }
    }
